...
INTERACTION_RELIABILITY_DENOMINATOR = 20


class InteractionEstimator(DurationEstimator):
    """유형과 난이도 조합별 계수를 표본 신뢰도에 따라 추가 반영한다."""
    ...
    def estimate(self, inp: EstimateInput) -> EstimateOutput:
        ...
        interaction_multiplier = inp.interaction_multiplier or 1.0

        slot_key = f"{inp.task_type.value}:{inp.difficulty}"
        dynamic_weights = normalize_weights({
            ...
            "interaction": reliability(
                inp.stats.slot_counts.get(slot_key, 0),
                INTERACTION_RELIABILITY_DENOMINATOR,
            ),
        })
        log_multiplier = (
            ...
            + dynamic_weights["interaction"] * log(interaction_multiplier)
        )
