...
| POST   | `/tasks/estimate`      | 태스크 예상 소요시간 산정                 | `services/initial_estimator/estimation.py` |
...
tests/services/
├── test_initial_estimator.py  # AVERAGE_BASELINE predict/update + 라우터 + drop
...
### 3.1 `/tasks/estimate` (태스크 예상 소요시간 산정)
  ...
  └─► api/routes/tasks.py         # /tasks/estimate 핸들러, Pydantic 검증
        └─► services/initial_estimator/estimation.py
              └─► initial_estimator/router.py  # completedCount 기반 단계 선택
                    └─► average_stage.predict()  # (또는 MAIN/INTERACTION)
                          → PredictResponse
              ← PredictResponse
        ← ApiResponse.ok(data=...)
```
...
- `average_stage.predict()`에서 `logCorrection`에 항을 더한다
...
- `test_initial_estimator.py`: AVERAGE_BASELINE predict/update + 라우터 + soft blending + drop (20+ 케이스)
