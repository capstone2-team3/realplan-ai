...
|--------|---------------------------|-------------------------------------------|----------------------------------------------|
| POST   | `/v1/tasks/classify`      | OpenAI 기반 태스크 유형 분류              | `services/classifier/`                       |
| POST   | `/v1/tasks/estimate`      | 태스크 예상 소요시간 산정                 | `services/predictor.py` → `planning_model/`  |
| POST   | `/v1/sessions/estimate`   | 세션 종료 시 잔여 소요시간 재계산         | `services/session_estimator.py`              |
| POST   | `/v1/users/planning-error-rates` | 완료 태스크 기반 사용자 계획오류율 갱신값 계산 | `services/updater.py`  → `planning_model/`   |
| POST   | `/v1/tasks/recommend`     | 특정 날짜의 태스크 추천도 계산            | `services/scheduler/`                        |
| POST   | `/v1/tasks/decompose`     | OpenAI 기반 태스크 세션 분할              | `services/task_decomposition.py`             |
| POST   | `/v1/schedules/auto-place` | 태스크 세션 자동 배치 계산               | `services/auto_placement.py`                 |
| GET    | `/health`                 | 헬스 체크                                 | (없음)                                       |
...
└── services/              # 순수 계산 로직
    ...
    ├── predictor.py       # /tasks/estimate 진입점 (라우터 호출)
...
### 3.1 `/v1/tasks/estimate` (태스크 예상 소요시간 산정)
...
### 3.2 `/v1/users/planning-error-rates` (계획오류율 갱신값 계산)
...
### 3.3 `/v1/sessions/estimate` (세션 잔여시간 재예측)
