...
|-------------|-------------------------------------|
| <= 0        | RULE only                           |
| 1 ~ 19      | RULE + AVERAGE_BASELINE soft blend   |
| 20 ~ 99     | AVERAGE_BASELINE only               |
| >= 100      | RIDGE_STUB 시도, 미구현이면 AVERAGE_BASELINE fallback |

`1 ~ 19` 구간의 blending은 `w_average = completed / (completed + 3)`로 계산한다.
...
분류 프롬프트는 태스크 이름만 입력으로 받고, 완료 기준이 시간인지, 객관적 분량인지, 주관적 완성도인지에 따라
`TIME_BASED` / `QUANTITY_BASED` / `SATISFACTION_BASED` 중 하나를 JSON으로 반환하도록 강제한다.
과거 유사 태스크가 있으면 프롬프트 호출 전에 `history_match` 결과를 우선 사용한다.

### 4.6 `app/services/task_recommendation/scheduler.py`

특정 일자에 수행하기 좋은 미완료 태스크를 최대 4개 추천한다. 마감 임박도,
작업량 압박, 중요도, 시간대별 집중도, 요구 집중도를 계산해 응답 DTO를 만든다.
최종 추천 점수는 `0.4 * deadlineUrgencyScore + 0.4 * workloadUrgencyScore + 0.2 * importanceScore`다.

### 4.7 `app/services/schedule_auto_completion/`

- `task_decomposition.py`: 추천 태스크를 요청의 `slotUnitMinutes` 단위 세션으로 자동 분할한다.
  OpenAI에는 `remainingMin - activeScheduledMin`을 슬롯 단위로 올린 `targetMinutes`와
  `title`, `memo`, `taskType`, `difficulty`만 전달한다.
  OpenAI 응답은 `taskId`, `sessionMinutes`만 만들고, `requiredFocusLevel`은 서버가 원본 difficulty에서 주입한다.
