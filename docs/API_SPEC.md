...
- 기한이 지난 태스크는 일반 태스크 그룹으로 분류하되, `workloadUrgencyScore=20`을 적용합니다.
...
`recommendScore = 0.7 * workloadUrgencyScore + 0.3 * importanceScore`
...
1. 오늘 마감 태스크를 `recommendScore` 높은 순으로 먼저 선택합니다.
2. 추천 개수가 4개 미만이면 일반 태스크(기한 지난/미래 마감/마감 없음)를 `recommendScore` 높은 순으로 추가합니다.
3. 최종 선택된 태스크 전체를 다시 `recommendScore` 내림차순으로 정렬합니다.
4. 동점은 `isDueToday=true`, 빠른 `dueDate`, 높은 `importanceScore`, 짧은 `remainingMin`, 작은 `taskId` 순으로 정렬합니다.
5. 정렬 후 `rank`를 1부터 다시 부여합니다.
