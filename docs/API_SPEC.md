...
| 값 | 전환 조건 |
|---|---|
| `STAGE_0` | `totalCompleted < 5` |
| `STAGE_1` | `5 <= totalCompleted < 15` |
| `STAGE_2` | `15 <= totalCompleted < 30` |
| `STAGE_3` | `30 <= totalCompleted < 60` |
| `STAGE_4` | `60 <= totalCompleted < 100` |
| `STAGE_5` | `totalCompleted >= 100` |
...
- Spring은 현재 태스크 key에 해당하는 단일 계수와 단일 count만 보내면 됩니다. 전체 map은 보내지 않습니다.
- `STAGE_0`에서는 개인화 로그 보정값을 사용하지 않고 `globalMultiplier`만 사용합니다.
- `STAGE_1` 이상에서는 로그 공간에서 `bias`, 1차 항, 2차 상호작용항을 단계별로 사용합니다.
...
| `coefficients.bias` | number | N | `0.0` | 없음 | 전역 로그 보정항 |
| `coefficients.globalMultiplier` | number | N | `1.0` | `> 0` | `STAGE_0` 예측에 사용하는 전역 배율 |
| `coefficients.folder` | number 또는 null | N | `null` | 없음 | 현재 `folderId` key의 계수 |
| `coefficients.difficulty` | number 또는 null | N | `null` | 없음 | 현재 difficulty key의 계수 |
| `coefficients.taskType` | number 또는 null | N | `null` | 없음 | 현재 taskType key의 계수 |
| `coefficients.folderDifficulty` | number 또는 null | N | `null` | 없음 | 현재 `{folderId}:{difficulty}` key의 계수 |
| `coefficients.folderType` | number 또는 null | N | `null` | 없음 | 현재 `{folderId}:{taskType}` key의 계수 |
| `coefficients.difficultyType` | number 또는 null | N | `null` | 없음 | 현재 `{difficulty}:{taskType}` key의 계수 |
...
| `counts.folderType` | integer | N | `0` | `>= 0` | 현재 `{folderId}:{taskType}` key의 완료 count |
| `counts.difficultyType` | integer | N | `0` | `>= 0` | 현재 `{difficulty}:{taskType}` key의 완료 count |
...
| `difficulty` | `"HARD"` |
| `taskType` | `"SCOPE_BOUND"` |
| `folderDifficulty` | `"10:HARD"` |
| `folderType` | `"10:SCOPE_BOUND"` |
| `difficultyType` | `"HARD:SCOPE_BOUND"` |

#### 예측 단계별 사용 항

| stage | 사용 항 |
|---|---|
| `STAGE_0` | `globalMultiplier`만 사용. `globalMultiplier`는 `0.5 ~ 2.5`로 clamp |
| `STAGE_1` | `bias`만 사용 |
| `STAGE_2` | `bias` + 1차 항. 1차 항은 count `>= 5`일 때 사용 |
| `STAGE_3` | `bias` + 1차 항 + 2차 항. 2차 항은 count `>= 8`, `interactionLambda=0.5` |
| `STAGE_4` | `bias` + 1차 항 + 2차 항. 2차 항은 count `>= 10`, `interactionLambda=1.0` |
| `STAGE_5` | `bias` + 1차 항 + 2차 항. 2차 항은 count `>= 15`, `interactionLambda=1.0` |

#### 신뢰도와 clamp

- `bias`, `globalMultiplier` reliability는 `1.0`입니다.
- 1차 항 reliability는 `n / (n + 10)`입니다.
- 2차 항 reliability는 `n / (n + 20)`입니다.
- `totalCompleted < 30`이면 `logCorrection`은 `-0.5 ~ 0.7`로 제한됩니다.
- `totalCompleted >= 30`이면 `logCorrection`은 `-0.7 ~ 0.9`로 제한됩니다.
...
| `usedTerms[].term` | string | Y | `bias`, `folder`, `difficulty`, `taskType`, `folderDifficulty`, `folderType`, `difficultyType`, `globalMultiplier` |
...
| `policy.minLogCorrection` | number 또는 null | Y | 적용된 하한. `STAGE_0`은 `null` |
| `policy.maxLogCorrection` | number 또는 null | Y | 적용된 상한. `STAGE_0`은 `null` |
...
- `counts.totalCompleted >= 5`이면 `globalMultiplier`는 변경하지 않고 로그 계수만 업데이트합니다.
...
| `coefficients` | object | N | 기본 계수 객체 | `globalMultiplier > 0` | `/v1/predict`와 동일한 scalar 구조 |
| `counts` | object | N | 기본 count 객체 | 값은 `>= 0` | `/v1/predict`와 동일한 scalar 구조 |

#### 업데이트 규칙

| stage | 업데이트 대상 |
|---|---|
| `STAGE_0` | `globalMultiplier`만 업데이트 |
| `STAGE_1` | `bias` |
| `STAGE_2` | `bias`, `folder`, `difficulty`, `taskType` |
| `STAGE_3` 이상 | `bias`, 1차 항, 2차 상호작용항 |

오차는 `log(actualMinutes / predictedMinutes)`로 계산하고, `log(0.5) ~ log(2.0)`으로 clamp합니다.

weight clamp:

| 항 | 범위 |
|---|---|
| `bias`, `folder`, `difficulty`, `taskType` | `-0.4 ~ 0.4` |
| `folderDifficulty`, `folderType`, `difficultyType` | `-0.3 ~ 0.3` |
| `globalMultiplier` | `0.5 ~ 2.5` |
  ...
  "success": {
    "data": {
      ...
      "error": {
        ...
        "predictedMinutes": 82,
        "actualMinutes": 95,
        "actualOverPredictedRatio": 1.1585,
        "logError": 0.1471,
        "clampedLogError": 0.1471
      },
      "updatedTerms": [
        {
          "term": "bias",
...
| `error.actualOverPredictedRatio` | number | Y | `actualMinutes / predictedMinutes` |
| `error.logError` | number | Y | 원본 로그 오차 |
| `error.clampedLogError` | number | Y | clamp된 로그 오차 |
...
| `updatedTerms[].share` | number | Y | 오차 배분 비율 |
| `updatedTerms[].reliability` | number | Y | 항별 신뢰도 |
