...
| `tasks[].finalEstimatedMinutes` | integer 또는 null | N | `null` | `> 0` | 최종 확정 예측 시간 |
...
| `tasks[].activeScheduledMinutes` | integer 또는 null | N | `0` | `>= 0` | 현재 유효하게 배치되어 있고 아직 실제 수행으로 반영되지 않은 시간 |
...
- 백엔드는 삭제/보관/권한 없는 태스크를 제외한 뒤 추천 계산에 필요한 원천 필드를 전달합니다.
- `status`는 `COMPLETED`, `PENDING`, `IN_PROGRESS` 중 하나여야 하며, `COMPLETED`이면 추천 후보에서 제외됩니다.
- Python 추천 API는 `finalEstimatedMinutes`, `aiEstimatedMinutes` 순서로 기준 시간을 선택합니다.
- `remainingMinutes = 기준 시간 - totalActualMinutes - activeScheduledMinutes`로 계산합니다.
...
3. 현재 유효한 미완료 배치 시간 합계를 `activeScheduledMinutes`로 계산해 전달합니다.
4. 추천 정책에 필요한 `taskId`, `title`, `dueDate`, `priority`, `status`, 예측 시간, 실제 수행 시간, 유효 예정 시간을 전달합니다.
5. `status = COMPLETED`, 기준 시간 없음, `remainingMinutes <= 0` 같은 추천 정책 필터링은 Python이 처리합니다.
