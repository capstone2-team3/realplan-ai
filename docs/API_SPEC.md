      ...
      "recommendations": [
        {
          ...
          "reason": "오늘 마감 태스크라 우선 추천되었습니다. 중요도가 높아 추천 점수에 크게 반영되었습니다. 오늘 마감 태스크라 가능한 이른 시간대인 오전을 추천했습니다."
        }
      ],
      "message": null
    }
...
- 기한이 지난 태스크는 일반 태스크 그룹으로 분류하되, `deadlineScore=20`으로 완전히 무시하지는 않습니다.
...
`reason`은 마감, 중요도, 추천 시간대, 잔여 시간, 요구 집중도 이유를 순서대로 계산한 뒤 앞의 3개 이유만 이어 붙입니다.

| 구분 | 조건 | 이유 |
|---|---|---|
| 마감 | 오늘 마감 | 오늘 마감 태스크라 우선 추천되었습니다. |
| 마감 | `deadlineScore >= 90` | 마감이 매우 임박해 추천 우선순위가 높습니다. |
| 마감 | `deadlineScore >= 70` | 마감이 가까워 미리 진행하는 것이 좋습니다. |
| 마감 | `deadlineScore >= 50` | 일주일 이내 마감 예정이라 추천 후보에 포함되었습니다. |
| 마감 | `deadlineScore <= 10` | 마감까지는 여유가 있지만 다른 조건을 함께 고려해 추천되었습니다. |
