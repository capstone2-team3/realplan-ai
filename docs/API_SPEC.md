...
Spring 백엔드가 전달한 태스크 정보, 보정계수, 완료 count를 기반으로 태스크 예상 소요시간을 계산합니다.

- Python FastAPI는 DB를 조회하거나 저장하지 않습니다.
- 현재 `/v1/tasks/estimate` 요청은 flat body 구조입니다.
...
- Spring 백엔드는 응답의 `updatedTerms`와 `countIncrements`를 사용해 사용자 계획오류율 관련 값을 DB에 저장합니다.
...
| `completedTask.predictedMinutes` | integer | Y | 없음 | `> 0` | `/v1/tasks/estimate`에서 반환한 예측 시간 |
...
| `coefficients` | object | N | 기본 계수 객체 | `globalMultiplier > 0` | `/v1/tasks/estimate`와 동일한 v2.1 계수 구조 |
| `counts` | object | N | 기본 count 객체 | 값은 `>= 0` | `/v1/tasks/estimate`와 동일한 count 구조 |
