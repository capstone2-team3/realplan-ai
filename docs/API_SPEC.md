...
| `plan_name` | string | Y | 플랜 이름. 단일 추천 시 `"생산성 최적"` |
...
`/v1/recommend`와 동일한 Knapsack 선택 결과를 기반으로 슬롯 배치 전략이 서로 다른 **3개 플랜**을 한 번에 반환합니다. 캘린더 UI에서 사용자가 원하는 플랜을 선택할 수 있도록 설계됐습니다.

| index | `plan_name` | 배치 전략 |
|---|---|---|
| 0 | `"생산성 최적"` | `difficulty` 내림차순 ↔ `predicted_focus` 내림차순 매칭. 어려운 태스크를 집중도 높은 슬롯에 배치. 집중도 보정 적용 |
| 1 | `"마감 우선"` | `days_until_deadline` 오름차순 → 시간순 슬롯에 순서대로 배치. 마감 가장 임박한 태스크가 가장 이른 슬롯 |
| 2 | `"워밍업"` | 가장 쉬운 태스크 → 가장 어려운 태스크(중간 슬롯) → 나머지 쉬운 태스크 순서로 시간순 슬롯에 배치 |

- `focus_slots`가 없거나 모두 `predicted_focus=0.0`이면 3개 플랜 모두 우선순위 기반 시간순 배치로 폴백합니다.
- 모든 플랜의 `items`(Knapsack 선택 조합)는 동일합니다. `schedule`만 플랜마다 다릅니다.
...
`/v1/recommend`와 동일합니다. `focus_slots` 포함 권장.
...
응답은 **배열**이며 항상 정확히 3개의 플랜 객체를 포함합니다.

| 필드 | 타입 | 필수 | 설명 |
|---|---|---:|---|
| `[].plan_name` | string | Y | 플랜 이름. `"생산성 최적"`, `"마감 우선"`, `"워밍업"` 중 하나 |
| `[].total_allocated_min` | integer | Y | 추천된 총 배정 시간 |
| `[].leftover_min` | integer | Y | 남은 가용시간 |
| `[].items` | array | Y | `/v1/recommend`의 `items`와 동일. 3개 플랜 모두 같은 값 |
| `[].schedule` | array | Y | 해당 플랜의 슬롯 배치 결과. Assignment 공통 타입 참조 |
