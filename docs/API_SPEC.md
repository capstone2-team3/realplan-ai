...
- 추천 시간대는 자동 배치 로직의 집중도 매칭 계산식을 공유하되, 시간대별 기본 focusScore를 사용합니다.
- 시간대별 기본 focusScore는 `06-12=85`, `12-18=65`, `18-24=45`입니다.
...
6. 추천 시간대 품질을 높이려면 `taskType`, `difficulty`도 함께 전달합니다.
7. `status = COMPLETED`, `remainingMin - activeScheduledMin <= 0` 같은 추천 정책 필터링은 Python이 처리합니다.
