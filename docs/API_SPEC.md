...
- `EARLY`에서는 `logAlphaGlobal`, `logAlphaType`, 난이도 prior를 사용합니다.
...
| `coefficients.globalMultiplier` | number | N | `1.0` | `> 0` | `logAlphaGlobal`이 없을 때 EARLY fallback으로 사용하는 전역 배율 |
| `coefficients.logAlphaGlobal` | number 또는 map | N | `null` | 없음 | EARLY 전역 로그 계수 |
| `coefficients.logAlphaType` | number 또는 map | N | `null` | 없음 | EARLY taskType 로그 계수 |
...
| `usedTerms[].term` | string | Y | `logAlphaGlobal`, `logAlphaType`, `difficultyPrior`, `betaIntercept`, `betaType`, `betaDifficulty`, `betaFolder`, 상호작용 beta 항 |
...
- `EARLY`에서는 `logAlphaGlobal`, `logAlphaType` EMA 업데이트 정보를 반환합니다.
...
| stage | 업데이트 대상 |
|---|---|
| `EARLY` | `logAlphaGlobal`, `logAlphaType` EMA 업데이트 |
