...
| POST | `/tasks/estimate` | 태스크 AI 예상 소요시간 산정 | `app/api/routes/tasks.py` |
...
- `previousAiTotalMinutes`에는 첫 세션 종료 시 최초 AI 예상값을, 이후에는 직전 응답의 `updatedAiTotalMinutes`를 넣습니다.
...
| `previousAiTotalMinutes` | number | Y | 없음 | 없음 | 직전 AI 예측 총 소요시간. 첫 세션 종료 시에는 최초 예상값, 이후에는 직전 `updatedAiTotalMinutes` 사용 |
