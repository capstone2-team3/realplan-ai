...
| `tasks[].remainingMinutes` | integer | Y | 없음 | `> 0` | 백엔드 `remainingMin`. 실제 수행 시간은 이미 차감된 남은 시간 |
...
- Python 추천 API는 `recommendableRemainingMinutes = remainingMinutes - activeScheduledMinutes`로 추천 가능한 남은 시간을 계산합니다.
...
3. `Task.remainingMin`을 `remainingMinutes`로 전달합니다.
4. 현재 유효한 미완료 배치 시간 합계를 `activeScheduledMinutes`로 계산해 전달합니다.
5. 추천 정책에 필요한 `taskId`, `title`, `dueDate`, `priority`, `status`, 남은 시간, 유효 예정 시간을 전달합니다.
6. `status = COMPLETED`, `remainingMinutes - activeScheduledMinutes <= 0` 같은 추천 정책 필터링은 Python이 처리합니다.
