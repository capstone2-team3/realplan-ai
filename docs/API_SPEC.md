...
| 값 | 설명 |
|---|---|
| `DISTRACTED` | 산만한 집중 상태. 보통 집중 기준으로 환산하면 잔여시간이 짧아짐 |
| `NORMAL` | 보통 집중 상태 |
| `FOCUSED` | 집중 상태. 보통 집중 기준으로 환산하면 잔여시간이 길어짐 |
| `FLOW` | 몰입 상태. 보통 집중 기준으로 환산하면 잔여시간이 더 길어짐 |
...
Spring 백엔드가 전달한 태스크 정보, 사용자별 보정 계수, 시스템 prior/effect를 기반으로 태스크 예상 소요시간을 계산합니다.
...
- `priority`, `systemPriorityEffect`는 legacy 필드이며 현재 초기 소요시간 예측 계산에는 사용하지 않습니다.
...
| `priority` | string 또는 null | N | `null` | 없음 | Legacy unused. 현재 초기 소요시간 예측 계산에는 사용하지 않음 |
...
| `systemPriorityEffect` | object | N | `{}` | 없음 | Legacy unused. 현재 초기 소요시간 예측 계산에는 사용하지 않음 |
...
## POST `/v1/users/planning-error-rates`
...
- `priority`, `systemPriorityEffect`는 legacy 필드이며 현재 업데이트 계산에는 사용하지 않습니다.
...
| `priority` | string 또는 null | N | `null` | 없음 | Legacy unused. 현재 업데이트 계산에는 사용하지 않음 |
...
| `systemPriorityEffect` | object | N | `{}` | 없음 | Legacy unused. 현재 업데이트 계산에는 사용하지 않음 |
...
```json
{
  ...
  "meta": {
    "timestamp": "2026-05-08T12:00:00",
    "path": "/v1/users/planning-error-rates"
  }
}
```
...
| `stage` | Prediction Stage | Y | 업데이트 단계. 현재 일반 업데이트는 `AVERAGE_BASELINE` |
| `dropped` | boolean | Y | 학습에서 제외되었는지 여부 |
| `dropReason` | string 또는 null | Y | drop 사유. drop되지 않았으면 `null` |

---

## POST `/v1/sessions/estimate`
...
|---|---:|---|
| `DISTRACTED` | 0.8 | 산만한 상태에서 측정된 잔여시간을 보통 집중 기준으로 짧게 환산 |
| `NORMAL` | 1.0 | 보통 집중 기준 |
| `FOCUSED` | 1.2 | 높은 집중 상태에서 측정된 잔여시간을 보통 집중 기준으로 길게 환산 |
| `FLOW` | 1.5 | 몰입 상태에서 측정된 잔여시간을 보통 집중 기준으로 더 길게 환산 |

---

## POST `/v1/tasks/recommend`
...
특정 날짜와 배치 가능 시간을 기준으로, 사용자가 그날 수행할 태스크 추천도와 권장 수행 시간을 계산합니다.
